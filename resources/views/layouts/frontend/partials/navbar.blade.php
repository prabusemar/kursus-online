<div class="w-full bg-[#BD562D] p-3 border-b border-[#403d39] border-dashed">
    <div class="container mx-auto">
        <div class="flex items-center justify-between">
            ...
            <ul class="flex items-center justify-center gap-5">
                <li>
                    <a href="/" class="flex items-center text-lg text-white font-semibold">
                        <img src="{{ asset('PPLG.png') }}" alt="PPLG Logo" class="w-10 h-10 mr-1">
                        SmartLearn
                    </a>
                </li>
                <li class="hidden lg:flex">
                    <a href="{{ route('home') }}"
                        class="text-sm font-semibold text-white flex items-center gap-2 hover:text-[#252422] {{ activeNav('home') }}">
                ...
                <li class="hidden lg:flex">
                    <a href="{{ route('course.index') }}"
                        class="text-sm font-semibold text-white flex items-center gap-2 hover:text-[#252422] {{ activeNav('course.index') }}">
                ...
                <li class="hidden lg:flex">
                    <div class="relative" x-data="{ isOpen: false }">
                        <button @click="isOpen = !isOpen" @keydown.escape="isOpen = false"
                            class="flex items-center gap-2 text-sm text-white hover:text-[#252422] {{ activeNav('category*') }}">
                            <svg xmlns="http://www.w3.org/2000/svg"
                        ...
                        <ul x-cloak x-show="isOpen" @click.away="isOpen = false"
                            class="absolute font-normal bg-[#252422] shadow overflow-hidden rounded-lg w-48 border border-[#403d39] mt-2 py-1 left-0 z-20">
                            @foreach ($categories as $category)
                                <li>
                                    <a href="{{ route('category', $category->slug) }}"
                                        class="flex items-center p-3 hover:text-[#BD562D] rounded-lg text-sm text-white">
                ...
                <li class="hidden lg:flex">
                    <a href="{{ route('review') }}"
                        class="text-sm font-semibold text-white flex items-center gap-2 hover:text-[#ccc5b9] {{ activeNav('review') }}">
                ...
                <li class="hidden lg:flex">
                    <a href="{{ route('showcase') }}"
                        class="text-sm font-semibold text-white flex items-center gap-2 hover:text-[#ccc5b9] {{ activeNav('showcase') }}">
                        <svg xmlns="http://www.w3.org/2000/svg"
            ...
            <div class="hidden md:flex items-center gap-2 text-white">
                @guest
                    <a href="{{ route('login') }}"
                        class="text-gray-200 font-semibold flex items-center border px-4 py-2 gap-2 rounded-lg text-xs bg-[#252422] border-[#ccc5b9]">
                    ...
                    <a href="{{ route('register') }}"
                        class="font-semibold text-white flex items-center border px-4 py-2 gap-2 rounded-lg text-xs bg-[#BD562D] border-[#333333]">
                ...
                @endguest
                @auth
                    <div class="rounded-lg border px-4 py-2 bg-[#252422] border-[#ccc5b9]">
                    ...
                    <div class="relative" x-data="{ isOpen: false }">
                        <button @click="isOpen = !isOpen" @keydown.escape="isOpen = false"
                            class="flex items-center gap-2 border px-4 py-2 rounded-lg text-sm bg-[#252422] border-[#ccc5b9]">
                            <img src="{{ Auth::user()->avatar }}" alt="avatar"
                                class="w-5 h-5 rounded-full border border-[#252422]">
                            {{ Auth::user()->name }}
                            <svg xmlns="http://www.w3.org/2000/svg" x-show="!isOpen"
                        ...
                        <ul x-cloak x-show="isOpen" @click.away="isOpen = false"
                            class="absolute font-normal bg-[#252422] shadow overflow-hidden rounded-lg w-48 border border-[#ccc5b9] mt-2 py-1 right-0 z-20">
                            @role('admin')
                                <li>
                                    <a href="{{ route('admin.dashboard') }}"
                                        class="p-3 rounded-lg text-sm font-semibold text-white flex items-center gap-2 hover:text-[#4A90E2]">
                                        <svg xmlns="http://www.w3.org/2000/svg"
                            ...
                            @endrole
                            <li class="border-b border-dashed border-[#ccc5b9]">
                                @role('admin')
                                    <a href="{{ route('admin.user.profile') }}"
                                        class="p-3 rounded-lg text-sm font-semibold text-white flex items-center gap-2 hover:text-[#BD562D]">
                                        <svg xmlns="http://www.w3.org/2000/svg"
